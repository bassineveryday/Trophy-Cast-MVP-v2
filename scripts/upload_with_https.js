...
function toJsonRow(headerCols, cols) {
  ...
  for (let i = 0; i < headerCols.length; i++) {
    ...
    if (n !== '' && !isNaN(Number(n))) obj[key] = Number(n);
...
function httpPostJson(fullUrl, headers, body) {
    ...
    const req = (isHttps ? https : http).request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (status >= 200 && status < 300) {
          try { const json = JSON.parse(data); resolve(json); } catch (e) { resolve(data); }
        } else {
          reject(new Error(`HTTP ${status} ${res.statusMessage}: ${data}`));
        }
      });
    });
...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node upload_with_https.js <input.csv> [batchSize] [table]'); process.exit(2);
  }
  ...
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    const cols = parseCsvLine(ln); while (cols.length < header.length) cols.push('');
    rows.push(toJsonRow(header, cols));
  }
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    try {
      const res = await httpPostJson(endpoint, headers, JSON.stringify(batch));
      if (Array.isArray(res)) uploaded += res.length; else uploaded += 0;
      console.log(`Batch OK: ${Array.isArray(res) ? res.length : 'response'}`);
    } catch (err) {
